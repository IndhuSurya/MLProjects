...
public class kmeans {
			...
			while ((csvLine = reader.readLine()) != null) {
				...
				while (count < attributes.length) {
					try {
						...
						dataset[i][count] = Double
								.parseDouble(attributes[count]);
	...
	private static void writeOutputFile(ArrayList<Centroid> points, int k,
			String outputFile) {
		PrintWriter writer;
		try {
	...
	private static double EuclidianDistance(int a, Centroid centroid) {
		...
		return (double) Math.abs(Math.sqrt(Math.pow(x1 - x2, 2)
				+ Math.pow(y1 - y2, 2)));
	}
