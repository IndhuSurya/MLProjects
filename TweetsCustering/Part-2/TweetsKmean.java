...
public class TweetsKmean {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((csvLine = reader.readLine()) != null) {
				...
				centroidArray = centroidArray[1]
						.split(", \"iso_language_code\":");
			...
			int index = 0;
			int i = 0;
			ArrayList<ArrayList<String>> selectedWords = new ArrayList<ArrayList<String>>(
					count);
			while (index < selectedLines.size()) {
				selectedLines.add(index,
						selectedLines.get(index).replace(",", " "));
				...
				selectedLines.remove(index + 1);
				selectedLines.add(index,
						selectedLines.get(index).replace("_", " "));
				selectedLines.remove(index + 1);
				selectedLines.add(index,
						selectedLines.get(index).replace("|", " "));
				selectedLines.remove(index + 1);
			...
			for(int u=0; u<251; u++)
			{
				for(int v=0; v<selectedWords.get(u).size(); v++)
				{
					if(selectedWords.get(u).get(v).equalsIgnoreCase(" "))
					{
						selectedWords.get(u).remove(v);
					}
					else if(selectedWords.get(u).get(v).equalsIgnoreCase(""))
					{
						selectedWords.get(u).remove(v);
					}
					else if(selectedWords.get(u).get(v).isEmpty())
					{
						selectedWords.get(u).remove(v);
					}
					else if(selectedWords.get(u).get(v).length()<=0)
					{
						selectedWords.get(u).remove(v);
					}
				}
				//System.out.println(selectedWords.get(u));
			}
			index = 0;
			// K-Means
			...
			for (int itr = 0; itr < 1; itr++) {
				for (int m = 0; m < selectedWords.size(); m++) {
					...
					for (int l = 0; l < noCluster; l++) {
						// finding index
						for (t = 0; t < 251; t++) {
							if (id.get(t).equalsIgnoreCase(
									cluster.get(l).getClusterId())) {
								break;
							}
						}
						if (minValue > jaccardDistance(selectedWords.get(m),
								selectedWords.get(t))) {
							// Assign the corresponding cluster
							minValue = jaccardDistance(selectedWords.get(m),
									selectedWords.get(t));
							minPosition = l;
						}
					}
				...
				for (int m = 0; m < noCluster; m++) {
					...
					while (j < cluster.get(m).getTweets().size()) {
						for (int s = 0; s < cluster.get(m).getTweets().size(); s++) {
							if (s != j) {
								minValue = minValue
										+ jaccardDistance(
												selectedWords.get(idPosition(m,
														s, id, cluster)),
												selectedWords.get(idPosition(m,
														j, id, cluster)));
							}
					...
					output.get(m).setClusterId(
							cluster.get(m).getTweets().get(minPosition));
					output.get(m).setTweets(cluster.get(m).getTweets());
					cluster.get(m).setClusterId(
							cluster.get(m).getTweets().get(minPosition));
			...
			writeOutputFile(output, noCluster, args[3]);
			System.out.println("SSE  "
					+ SSE(output, noCluster, selectedWords, id));
		} catch (IOException e) {
	...
	private static int idPosition(int m, int j, ArrayList<String> id,
	...
	private static double jaccardDistance(ArrayList<String> a,
			ArrayList<String> b) {
	...
	private static double SSE(ArrayList<Cluster> output, int k,
			ArrayList<ArrayList<String>> selectedWords, ArrayList<String> id) {
		double sum = 0.0;
		int t = 0;
		for (int m = 0; m < k; m++) {
			for (t = 0; t < 251; t++) {
			...
			for (int i = 0; i < output.get(m).getTweets().size(); i++) {
				// System.out.println(jaccardDistance(selectedWords.get(idPosition(m,
				// i, id, output)), selectedWords.get(t)));
				sum = sum
						+ Math.pow(
								jaccardDistance(selectedWords.get(idPosition(m,
										i, id, output)), selectedWords.get(t)),
								2);
			}
		}
	...
	private static void writeOutputFile(ArrayList<Cluster> output,
			int noCluster, String outputFile) {
